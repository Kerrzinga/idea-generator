...
// Picks today's six adjectives and six characters (one per die face),
// reshuffled from the full lists using today's date as the seed.
export function getDailyWords(): DailyWords {
  const dateKey = todayKey();
  const adjectives = seededShuffle(ADJECTIVES, hashStringToInt(`${dateKey}-adjective`)).slice(0, DIE_SIDES);
  const characters = seededShuffle(CHARACTERS, hashStringToInt(`${dateKey}-character`)).slice(0, DIE_SIDES);
